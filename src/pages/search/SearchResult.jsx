...
export default function SearchResult() {
  ...
  return (
    <div className="flex flex-col h-screen bg-white font-[Pretendard]">
          ...
          {results.map((item) => (
            <div key={item.id} className="pb-[1.25rem] mb-[1.25rem]">
              ...
              <div className="flex justify-between items-center mt-[0.8rem]">
                <button onClick={() => toggleLike(item.id)} className="flex items-center gap-[0.25rem]">
                  <img
                    src={
                      likes[item.id]
                ...
                <button
                  onClick={() => toggleParticipate(item.id)}
      ...
      {/* 질문하기 버튼 */}
      <button
        className="fixed bottom-[5.5rem] right-[1.5rem] w-[7rem] h-[2.75rem] bg-[#FA502E] text-white rounded-[1.5rem] text-[0.875rem] font-medium shadow-md flex items-center justify-center z-50"
